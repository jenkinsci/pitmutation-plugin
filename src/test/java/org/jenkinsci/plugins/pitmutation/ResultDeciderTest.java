...
class ResultDeciderTest {
    ...
    Stream<DynamicTest> testFactory() {
        ...
        Collection<Object[]> inputs = Arrays.asList(new Object[][]{{SUCCESS, SUCCESS, SUCCESS},
        ...
        return inputs
            .stream()
            .map(input -> DynamicTest.dynamicTest("Testing with thresholdResult = "
                                                  + input[0]
                                                  + ", mustImproveResult = "
                                                  + input[1],
                                                  () -> runTest((Result) input[0],
                                                                (Result) input[1],
                                                                (Result) input[2])));
    }
